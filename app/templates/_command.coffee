...
MISSING_SERVICE_URL = 'Missing required environment variable: SLURRY_<%= constantPrefix %>_SERVICE_URL'
MISSING_MANAGER_URL = 'Missing required environment variable: SLURRY_<%= constantPrefix %>_MANAGER_URL'
...
class Command
  getOptions: =>
    ...
    throw new Error MISSING_SERVICE_URL if _.isEmpty process.env.SLURRY_<%= constantPrefix %>_SERVICE_URL
    throw new Error MISSING_MANAGER_URL if _.isEmpty process.env.SLURRY_<%= constantPrefix %>_MANAGER_URL
    ...
    return {
      ...
      serviceUrl:      process.env.SLURRY_<%= constantPrefix %>_SERVICE_URL
      userDeviceManagerUrl: process.env.SLURRY_<%= constantPrefix %>_MANAGER_URL
      staticSchemasPath: process.env.SLURRY_<%= constantPrefix %>_STATIC_SCHEMAS_PATH
    }
